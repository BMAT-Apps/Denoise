...
def bids_denoise(bids, sub, ses, ref_path, deriv='Denoise', out_name=None):
    ...
    if not out_name:
        ...
        img_d.insert(-1, 'den-ants')
        out_name = '_'.join(img_d)
        
    subprocess.Popen(f"DenoiseImage -i {pjoin(img_p, img_in)} -o {pjoin(sub_ses_deriv, f'{out_name}.nii.gz')}", shell=True).wait()
    
  
def bids_denoise_docker(bids, sub, ses, ref_path, deriv='Denoise', out_name=None):
    ...
    if not out_name:
        img_d = img_in.split('_')
        img_d.insert(-1, 'den-ants')
        out_name = '_'.join(img_d)
        
    subprocess.Popen(f"docker run --rm --privileged -v {img_p}:/media/input -v {sub_ses_deriv}:/media/output antsx/ants DenoiseImage -i {pjoin('/media/input', img_in)} -o {pjoin('/media/output', f'{out_name}.nii.gz')}", shell=True).wait()
...
if __name__ == '__main__':
    ...
    description = '''
bids_denoise:
    Denoise Image with non-local adaptive means from ANTs  
    '''
    ...
    parser.add_argument('--derivative', '-d', dest='deriv', type=str, help='derivative folder name to store the output (default: Denoise)', default='Denoise', required=False)
    parser.add_argument('--out-name', '-o', dest='out_name', type=str, help='name of the output sequence name (default: add den-ants to input img name)', default=None, required=False)
    ...
    for sub, sess in subjects_and_sessions:
        for ses in sess:
            ...
            if args.use_docker:
                bids_denoise_docker(bids, sub, ses, args.ref_path, deriv=args.deriv, out_name=args.out_name)
            else:
                bids_denoise(bids, sub, ses, args.ref_path, deriv=args.deriv, out_name=args.out_name)
