...
from bids_denoise import bids_denoise, bids_denoise_docker, find_subjects_and_sessions
import time
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent, add_info):
        ...
        self.pipeline = "DenoiseImage"
        
        sss_slurm = self.add_info.get('sss_slurm')
        
        if sss_slurm == None:
            print('no sss slurm')
            self.tab = DenoiseTab(self, sss_slurm)
            layout = QVBoxLayout()
            ...
            self.main_tab = DenoiseTab(self, sss_slurm)
...
class DenoiseTab(QWidget):
    ...
    def __init__(self, parent, sss_slurm=None):
        ...
        self.select_sequence_to_denoise_button = QPushButton("Select image to denoise")
        self.select_sequence_to_denoise_button.clicked.connect(self.select_sequence_to_denoise)
        self.sequence_to_denoise_label = QLabel()
        
        self.deriv = QLineEdit(self)
        self.deriv.setPlaceholderText("output derivative folder (default: Denoise)")
        
        self.out_name = QLineEdit(self)
        self.out_name.setPlaceholderText("output img name (default: add den-ants to input image)")
        ...
        self.button = QPushButton("Denoise Image")
        self.button.clicked.connect(self.action)
        
        layout = QGridLayout()
        layout.addWidget(self.select_sequence_to_denoise_button, 0, 0, 1, 1)
        layout.addWidget(self.sequence_to_denoise_label, 0, 1, 1, 1)
    ...
    def select_sequence_to_denoise(self):
        """
        ...
        """
        self.ref_path, _ = QFileDialog.getOpenFileName(self, "Select image to Denoise", self.bids.root_dir, options=QFileDialog.DontUseNativeDialog)
        if self.ref_path == '':
            return
        ...
        self.sequence_to_denoise_label.setText(seq_name)
    ...
    def action(self):
        ...
        if self.local:
            ...
            self.thread = QThread()
            self.action = ActionWorker(self.bids.root_dir, sub, ses, self.pipeline, self.ref_path, deriv=deriv, out_name=out_name, use_docker=use_docker)
            ...
            self.is_in_progress(('DenoiseImage', True))
            jobs_submitted = []
            ...
            self.is_in_progress(('DenoiseImage', False))
...
class ActionWorker(QObject):
    ...
    def __init__(self, bids, sub, ses, pipeline, ref_path, deriv=None, out_name=None, use_docker=False):
        """
    ...
    def run(self):
        ...
        for sub, sess in subjects_and_sessions:
            for ses in sess:
                ...
                if self.use_docker:
                    print('use_docker')
                    bids_denoise_docker(self.bids, sub, ses, self.ref_path, **kwargs)
                else:
                    bids_denoise(self.bids, sub, ses, self.ref_path, **kwargs)
